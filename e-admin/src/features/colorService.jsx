...
const colorPatch=async(userData)=>{
    const response=await axios.patch(`${URL.BASE_URL}color/colorUpdates/${userData}`,userData)
    if(response.data){
        return response.data
    }
}
...
const colordelete=async(userData)=>{
    const response=await axios.delete(`${URL.BASE_URL}color/colorDelete/${userData}`,userData)
    if(response.data){
        return response.data
    }
}
...
const colorfind=async(userData)=>{
    const response=await axios.get(`${URL.BASE_URL}color/colorDelete/${userData}`)
    if(response.data){
        return response.data
    }
}
